package net.yhte.javascript
{
	...
	public class JsAdapter
	{
		 ...
		 * @private
		 */ 
		public function JsAdapter(sprite:Sprite)
		{
			if ( instance != null)
			{
			...
			var appId:String = AS_OBJ + new Date().time;
			regist(sprite,appId);
			
			var temp:Array = FlexGlobals.topLevelApplication.url.split("/");
			var name:String = temp[temp.length-1].replace(".swf","");
			
			//声明Ao对象
			ExternalInterface.call("eval",SCRIPT);
			
			var script:String = '(function(){var a = document["%name%"]||window["%name%"];Ao.init(a);Ao.application=new Ao("%appId%");}());'.replace(/%name%/g,name).replace(/%appId%/g,appId);
			ExternalInterface.call("eval",script);
		}
		 ...
		 */
		public static function  getInstance(sprite:Sprite):JsAdapter
		{
			if (instance == null)
			{
		...
		private function R(node:String,klass:String):Array
		{
			var xml:XML = reflect(klass);
			var temp:Array = [];
			for each(var m:* in xml[node]['@name'])
			{
